...
namespace Com.TheFallenGames.OSA.Core
{
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		public void Init()
		{
			...
			if (!enabled)
			{
				throw new OSAException("Init should only be called when OSA component is enabled");
			}

			if (!gameObject.activeInHierarchy)
			{
                throw new OSAException("Init should only be called when the GameObject is active in hierarchy");
            }
		...
		public virtual void ScrollTo(int itemIndex, float normalizedOffsetFromViewportStart = 0f, float normalizedPositionOfItemPivotToUse = 0f)
		{
			...
			{
				Debug.Log("_InternalState is null");
			}
